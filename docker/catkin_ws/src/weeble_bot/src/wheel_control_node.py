...
from weeble_bot_util.msg import SensorsRaw, WheelSpeeds, MotorCommands, WheelAngles, WheelDisplacements
...
#==============================================================================
    ...
    sub_wheel_speeds = rospy.Subscriber('/wheel_speeds_desired', WheelSpeeds, set_wheel_speed_targets,[mc0,mc1])  
...
def set_wheel_speed_targets(msg_in, motor_controllers):
    motor_controllers[0].update_target_value(msg_in.speed_left)
    motor_controllers[1].update_target_value(msg_in.speed_right)
    
def encoder_update(quad_encoders, mot_controllers):
    # Create the publisher for the topic "/motor_commands", with message type "MotorCommands"
    pub_motorcommands = rospy.Publisher('/motor_commands', MotorCommands, queue_size=10)
    # Create the publisher for the topic "/robot_wheel_angles", with message type "WheelAngles"    
    pub_robot_wheel_angles = rospy.Publisher('/robot_wheel_angles', WheelAngles, queue_size = 10)
    # Create the publisher for the topic "/robot_wheel_displacements", with message type "WheelDisplacements"    
    pub_robot_wheel_displacements = rospy.Publisher('/robot_wheel_displacements', WheelDisplacements, queue_size = 10)


    #Make messages to fill later.
    pub_motorcommands_message = MotorCommands()
    pub_motorcommands_message.cmd0 = 0
    pub_motorcommands_message.cmd1 = 0

    robot_wheel_angles_message = WheelAngles()
    robot_wheel_angles_message.ang_left = 0. 
    robot_wheel_angles_message.ang_right = 0. 

    robot_wheel_displacements_message = WheelDisplacements()
