...
set -e
sudo apt-get update
sudo apt-get -y install bluez  checkinstall libbluetooth-dev libusb-dev joystick git libjack-jackd2-dev
git clone -b sdp-error-workaround https://github.com/johntrimble/qtsixa.git
cd qtsixa/sixad
...
sudo make install
cd ../..
echo "sudo ./qtsixa/utils/bins/sixpair" >> pair-controller.sh
